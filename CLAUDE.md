...
# Run the instrumented playback tests (needs a connected device)
./project_01_android/gradlew -p ./project_01_android connectedDebugAndroidTest
# CAREFUL: this installs the debug APK and has been seen to wipe the app's stored files —
# i.e. a phone's pre-loaded videos. Never run it against a phone carrying a pre-load you need.
...
- **`STATE_ENDED` only happens for the LAST video in the playlist.** Measured on an S9 (media3 1.2.1) — at the end of a non-last item `pauseAtEndOfMediaItems` reports `onPlayWhenReadyChanged(false, PLAY_WHEN_READY_CHANGE_REASON_END_OF_MEDIA_ITEM)` and stays in `STATE_READY`, with the index still on the video that just finished; only the last item reaches `STATE_ENDED`. Both signals matter and they arrive differently:
  - the **intent flip to paused** (which shows the blue safe-screen) comes from `onIsPlayingChanged`, and additionally from `onPlaybackStateChanged(STATE_ENDED)` for the last video, whose `playWhenReady` change would otherwise be missed;
  - the **"this video is finished" fact** that `advanceOrResume` needs is tracked by `ui/EndOfVideoTracker`, which watches END_OF_MEDIA_ITEM *and* STATE_ENDED. See that file for the field failure it fixes.
...
- **`isAtEndOfCurrent()` asks `EndOfVideoTracker`**, not `playbackState == STATE_ENDED` and not `position >= duration - 500ms`. The position heuristic misfired on short videos (a 2s clip counts as "at the end" for a quarter of its length) and mid-seek; reading STATE_ENDED alone was false at the end of every video except the last, so Play *resumed the finished video* instead of advancing. That produced three symptoms at once and cost a field test on 2026-08-17: (1) the game master's own ExoPlayer, still parked from its own END_OF_MEDIA_ITEM, rolls into the next video when `playWhenReady` goes true again — so the GM looked fine; (2) the players, which the preceding pause command had given an explicit `seekTo(index, endPosition)`, played out the last milliseconds, parked again on the same item and sat on a **frozen frame of the previous video** (both behaviours measured on an S9); (3) intent's index then trailed ExoPlayer's by one, so the last video's real `STATE_ENDED` was dropped by the index guard below and the playlist never returned to blue. Only reproducible when a non-last video is allowed to finish — interrupting it commits a correct index through the normal mutators.
...
- 295 unit tests across the codebase.
- `EndOfVideoTrackerTest` (plain JVM, media3 constants are compile-time ints so no Robolectric) walks the end-of-video state machine, including the exact sequence of the 2026-08-17 "frozen frame instead of the next video" failure.
- **Instrumented tests** (`app/src/androidTest/`, run with `connectedDebugAndroidTest`, device required): `ExoPlayerEndOfItemContractTest` asserts what ExoPlayer really does at the end of a video — non-last videos report `END_OF_MEDIA_ITEM` and stay READY, only the last reaches STATE_ENDED, a parked video rolls forward on resume, and a video seeked to its end does not. Those four facts are what the GM's Play button is built on, they are not derivable from the docs, and getting one wrong has cost a field test. The last test in the file drives the real `EndOfVideoTracker` from real callbacks as a regression guard (verified to fail when the fix is reverted, alongside 3 of the JVM tests).
- Its media comes from 2-second solid-colour clips in `app/src/androidTest/assets/`, generated by `tools/maketestvideo` (macOS/Swift; see its README). ExoPlayer needs real media to answer these questions, so the clips are checked in — ~5KB each, no camera footage or network needed.
